...
const token = tokenType => (
  p.expected(p.sat(x => x.type == tokenType))
);
const identifier = token("identifier")

const tokenParser = p.seq(function*(){
  yield token("letKeyword");
  const {value: ident} = yield identifier;
  return ident.capture;
});

const ast2 = p.parse(tokenParser, p.stream(tokens))
console.log(ast2.value);
